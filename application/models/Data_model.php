...
class Data_model extends CI_Model
{
	...
	public function datainv()
	{
		$query = "SELECT * FROM `data` LEFT JOIN `data2` ON `data2`.`id` = `data`.`id` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` LEFT JOIN `kecamatan` ON `kecamatan`.`id_kecamatan` = `data`.`id_kecamatan` LEFT JOIN `opd` ON `opd`.`id` = `data`.`opd` LEFT JOIN `jointing` ON `jointing`.`id` = `datagis`.`jointing` ORDER BY `data`.`id` ASC
		";
	...
	public function opd_tampil() {
		$query = "SELECT * FROM `opd` WHERE `name_gis` = '2' 
		";
