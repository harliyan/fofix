	...
	// end search
		...
		public function get_konverter_s() {
			$sql = " SELECT count(*) as count  FROM `data` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` where jenis_konverter = 'single' " ;
			$data = $this->db->query($sql);
			return $data->result();
		}

		public function get_konverter_d() {
			$sql = " SELECT count(*) as count  FROM `data` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` where jenis_konverter = 'double' " ;
			$data = $this->db->query($sql);
			return $data->result();
		}

		public function get_up() {
			$sql = " SELECT count(*) as count  FROM `data` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` where kondisi = 'up' " ;
			$data = $this->db->query($sql);
			return $data->result();
		}

		public function get_down() {
			$sql = " SELECT count(*) as count  FROM `data` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` where kondisi = 'down' " ;
			$data = $this->db->query($sql);
			return $data->result();
		}
