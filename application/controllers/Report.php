...
class Report extends CI_Controller
{
        ...
        public function barat()
    {
     ...
     public function cetakpdf(){
        ...
        $this->pdf->Image('assets/img/pekalongan2.png',0.5,0.5,-300);
        ...
        $report = $this->db->query("SELECT * FROM `data` LEFT JOIN `data2` ON `data2`.`id` = `data`.`id` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` LEFT JOIN `kecamatan` ON `kecamatan`.`id_kecamatan` = `data`.`id_kecamatan` LEFT JOIN `opd` ON `opd`.`id` = `data`.`opd`  WHERE `data`. `gis` = '2'")->result();
    ...
    public function cetakpdfutara(){
        ...
        $report = $this->db->query("SELECT * FROM `data` LEFT JOIN `data2` ON `data2`.`id` = `data`.`id` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` LEFT JOIN `kecamatan` ON `kecamatan`.`id_kecamatan` = `data`.`id_kecamatan` LEFT JOIN `opd` ON `opd`.`id` = `data`.`opd`  WHERE  `data`. `id_kecamatan` = '1' AND `data`. `gis` = '2'")->result();
      ...
      public function cetakpdftimur(){
       ...
       $report = $this->db->query("SELECT * FROM `data` LEFT JOIN `data2` ON `data2`.`id` = `data`.`id` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` LEFT JOIN `kecamatan` ON `kecamatan`.`id_kecamatan` = `data`.`id_kecamatan` LEFT JOIN `opd` ON `opd`.`id` = `data`.`opd`  WHERE  `data`. `id_kecamatan` = '2' AND `data`. `gis` = '2'")->result();
      ...
      public function cetakpdfselatan(){
        ...
        // $report = $this->db->get_where('data', array('id_kecamatan' => 3))->result();
         $report = $this->db->query("SELECT * FROM `data` LEFT JOIN `data2` ON `data2`.`id` = `data`.`id` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` LEFT JOIN `kecamatan` ON `kecamatan`.`id_kecamatan` = `data`.`id_kecamatan` LEFT JOIN `opd` ON `opd`.`id` = `data`.`opd`  WHERE  `data`. `id_kecamatan` = '3' AND `data`. `gis` = '2'")->result();
        // $report = $this->db->get('data')->result();
        foreach ($report as $row){
            $this->pdf->ln();
            $this->pdf->Cell(0.8,0.8, $row->id,'LTR',0,'C',0);   
      ...
      public function cetakpdfbarat(){
       ...
       $report = $this->db->query("SELECT * FROM `data` LEFT JOIN `data2` ON `data2`.`id` = `data`.`id` LEFT JOIN `datagis` ON `datagis`.`id` = `data`.`id` LEFT JOIN `cluster` ON `datagis`.`cluster` = `cluster`.`id` LEFT JOIN `kecamatan` ON `kecamatan`.`id_kecamatan` = `data`.`id_kecamatan` LEFT JOIN `opd` ON `opd`.`id` = `data`.`opd`  WHERE  `data`. `id_kecamatan` = '4' AND `data`. `gis` = '2'")->result();
        // $report = $this->db->get('data')->result();
        foreach ($report as $row){
            $this->pdf->ln();
            $this->pdf->Cell(0.8,0.8, $row->id,'LTR',0,'C',0);   
